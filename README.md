...
**현재 우선순위:**
1. ✅ **하드웨어**: ESP32 + OV2640 카메라 구성 및 배선
2. ✅ **백엔드**: Orange Pi RV2 서버 (FastAPI + YOLO)
3. ✅ **데이터베이스**: 인원수 데이터 저장 구조
4. 🔜 **프론트엔드**: 웹 대시보드 (추후 개발 예정)
...
4. 이벤트 DB에 저장
5. (예정) 사용자 앱/웹 대시보드에서 실시간 혼잡도 표시
6. (예정) 변동 폭이 특정 기준 초과하면 알림 또는 보상 로직 실행(플랫의 '혼잡도 크리에이터' 기능)
...
## 7. 웹 대시보드 구조 (🔜 추후 개발 예정)

> **우선순위**: 후순위 - 백엔드(FastAPI + YOLO) 및 하드웨어(ESP32 + 카메라) 완성 후 개발
...
**데이터 페칭:**
- TanStack Query (React Query) - 캐싱 및 상태 관리
- 수동 새로고침 버튼 제공
