...
try:
    ...
    try:
        ...
        # imsave는 pilutil에 없을 수 있으므로 imageio 사용
        try:
            import imageio
            ...
            def imsave(filename, arr):
                ...
                if len(arr.shape) == 3 and arr.shape[0] == 3:  # CHW format
                    arr = arr.transpose(1, 2, 0)  # HWC format
                Image.fromarray(arr).save(filename)
    except ImportError:
        # scipy.misc.pilutil도 없으면 imageio 사용
        try:
            import imageio
